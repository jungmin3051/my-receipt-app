...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        # --- [정밀 추출 로직] ---
        # 1. 날짜 추출 (매출일시 근처에서 찾기)
        date_match = re.search(r'(?:매출일|일시|날짜)\s*[:]?\s*(\d{4}[-/.]\d{2}[-/.]\d{2})', raw_text)
        if not date_match: # 없으면 일반 날짜 형식 찾기
        ...
        meal_type = "중식"; extracted_time = "12:00:00"
        if time_match:
        ...
        # 3. 금액 추출 (합계금액, 받을금액 등)
        price_match = re.search(r'(?:합계|받을|결제|금\s*액)\s*[:]?\s*([\d,]{3,})', raw_text.replace(' ', ''))
        ...
        with st.form(key=f"form_{uploaded_file.name}"):
            ...
            if st.form_submit_button("확정 및 목록 추가"):
                data_list.append([date, store, meal, price, remark])
                st.success(f"{store} 추가 완료!")
