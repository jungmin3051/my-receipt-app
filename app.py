...
# --- 1단계: 사진 업로드 및 데이터 추출 ---
st.subheader("📸 1단계: 영수증 사진 전송")

# 폰 앱에서 바로 카메라를 호출합니다
img_file = st.camera_input("영수증을 촬영해주세요")

if img_file:
    # 촬영된 사진 미리보기
    st.image(img_file, caption="촬영된 사진", width=300)
    
    # 전송 버튼 (클릭 시 구글 시트에 대기 상태로 저장)
    if st.button("🚀 사진 전송하기", type="primary", use_container_width=True):
        try:
            # 이미지 변환 (Base64)
            img_b64 = img_to_base64(Image.open(img_file))
            now = datetime.now()
            
            # 새 데이터 생성 (날짜는 오늘, 상태는 대기)
            new_data = {
            ...
            # 기존 데이터와 합쳐서 업데이트
            updated_df = pd.concat([all_data, pd.DataFrame([new_data])], ignore_index=True)
            conn.update(spreadsheet=SHEET_URL, worksheet="Sheet1", data=updated_df[COLUMNS])
            
            st.success("시트로 전송되었습니다! 2단계에서 내용을 수정해주세요.")
            st.cache_data.clear()
            time.sleep(1)
            st.rerun()
