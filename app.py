...
# --- [유틸리티 함수 정의 영역] : 데이터 로드보다 위에 있어야 에러가 안 납니다 ---
...
try:
    # 데이터 로드 시 ttl=0으로 설정하여 캐시 문제 방지
...
with st.expander("📸 1단계 : 사진 업로드", expanded=True):
    ...
    if files and st.button("🚀 사진 전송"):
        ...
        for f in files:
            ...
            except Exception as e: st.error(f"사진 처리 오류: {e}")
        ...
        if new_list:
            ...
            st.cache_data.clear()
            st.success(f"{len(new_list)}장의 사진이 업로드되었습니다!")
            time.sleep(1)
            st.rerun()
...
if not all_data.empty:
    ...
    idx = st.selectbox("수정할 항목 선택", range(len(row_list)), index=curr_idx,
                       format_func=lambda x: f"[{x+1}] {row_list[x]['날짜']} | {row_list[x]['식당명']} ({row_list[x]['상태']})")
    ...
    with c1: 
        if row["사진데이터"]:
            st.image(base64.b64decode(row["사진데이터"]), use_column_width=True)
    ...
    with c2:
        ...
        with f2:
            ...
            u_price = st.text_input("금액 (숫자만 입력)", value="" if is_pending else row["금액"])
        u_note = st.text_input("비고", value="" if is_pending else row["비고"])
        
        if st.button("💾 이 항목 저장", use_container_width=True, type="primary"):
...
if not all_data.empty:
    ...
    <div style='background-color:#f8f9fb;padding:12px;border-radius:10px;border:1px solid #e6e9ef;margin:10px 0;'>
        <div style='display:flex;justify-content:space-around;align-items:center;'>
            <div style='text-align:center;'><span style='font-size:14px;color:#666;'>💳 총 사용 금액</span><br><span style='font-size:22px;font-weight:bold;'>{total_sum:,} 원</span></div>
            <div style='width:1px;height:35px;background-color:#e6e9ef;'></div>
            <div style='text-align:center;'><span style='font-size:14px;color:#666;'>💰 총 남은 한도</span><br><span style='font-size:22px;color:{remain_color};font-weight:bold;'>{remaining_amount:,} 원</span></div>
        </div>
    </div>
    """
