...
def fix_date(d):
    """날짜 형식을 2026-03-11 형태로 통일하여 정렬 오류 방지"""
    d_str = str(d).strip()
    if len(d_str) == 8 and d_str.count('-') == 2:
        return "20" + d_str
    return d_str
...
def create_photo_pdf(df):
    """[핵심] PDF 생성: 한글 폰트 적용 및 하단 상세정보 출력"""
    pdf = FPDF()
    
    # 깃허브에 올린 폰트 파일 경로 (파일명 대소문자 확인 필수)
    font_path = "NanumGothic.ttf"
    
    if os.path.exists(font_path):
        pdf.add_font('Nanum', '', font_path, uni=True)
        pdf.set_font('Nanum', size=8) # 하단 정보는 작은 글씨로
    else:
...
if not all_data.empty:
    ...
    idx = st.selectbox(
        "수정할 항목 선택", range(len(row_list)), index=curr_idx,
    ...
    with c2:
        ...
        if st.button("💾 정보 저장 및 다음으로", use_container_width=True):
...
if not all_data.empty:
    ...
    if selected_rows and st.button("🗑️ 선택 항목 삭제", type="primary"):
...
if not done_df.empty:
    st.subheader("📥 4단계: 보고서 다운로드")
    d1, d2 = st.columns(2)
    with d1:
        ex_out = io.BytesIO()
        done_df.drop(columns=["사진데이터", "상태"]).to_excel(ex_out, index=False)
        st.download_button("📊 엑셀 저장", ex_out.getvalue(), "법카내역.xlsx", use_container_width=True)
    with d2:
        pdf_fn = f"{datetime.now().month}월 영수증_한정민.pdf"
        st.download_button("📄 PDF 저장 (한글패치)", create_photo_pdf(done_df), pdf_fn, use_container_width=True)
