...
if not all_data.empty:
    ...
    # [좌표 고정] CSS로 '삭제체크' 헤더 바로 옆으로 체크박스 강제 이동
    st.markdown("""
        <style>
        /* 체크박스 전체 컨테이너를 표 헤더 높이에 맞춰 강제 배치 */
        div[data-testid="stHorizontalBlock"]:has(input[aria-label=""]) {
            position: relative;
            margin-bottom: -50px; /* 표 위로 겹치게 올림 */
            z-index: 100;
        }
        /* 네모칸만 따로 떼서 '삭제체크' 옆으로 밀기 */
        div[data-testid="stHorizontalBlock"] .stCheckbox {
            position: absolute;
            right: 25px; /* 오른쪽 끝에서 헤더 글자 옆으로 간격 조절 */
            top: 15px;
        }
        </style>
    """, unsafe_allow_html=True)

    # 실제 체크박스 생성 (표 위에 겹쳐서 배치됨)
    _, col_check = st.columns([9.3, 0.7]) 
    with col_check:
        select_all = st.checkbox("", key="final_absolute_sel")
