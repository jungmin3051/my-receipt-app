...
# [수정] 압축 강도 조절 (해상도 1000, 화질 60으로 상향)
...
if not all_data.empty:
    st.divider()
    st.markdown("<h3 style='text-align: center;'>👀 3단계 : 내역 확인 및 삭제</h3>", unsafe_allow_html=True)
    ...
    # [수정] 모든 컬럼 중앙 정렬 설정
    cols_to_center = ["날짜", "식당명", "시간대", "금액", "비고", "상태"]
    configs = {c: st.column_config.TextColumn(alignment="center") for c in cols_to_center}
    configs["삭제체크"] = st.column_config.CheckboxColumn(alignment="center")

    edited_data = st.data_editor(
        edit_df, 
        use_container_width=True, 
        disabled=cols_to_center,
