...
if not all_data.empty:
    ...
    # [수정] 표의 '삭제체크' 열 위치와 수직으로 맞추기 위해 컬럼 비율 조정
    # 오른쪽 끝으로 네모칸을 보내기 위해 왼쪽 여백(9.5)을 크게 잡았습니다.
    cols = st.columns([9.5, 0.5]) 
    with cols[1]:
        # 글자 없이 네모칸만 딱! 
        select_all = st.checkbox("", key="final_square_only")
