...
# Google Sheets 연결 (비밀번호/토큰은 .streamlit/secrets.toml에 있어야 함)
...
def create_photo_pdf(df):
    ...
    df_copy = df.copy()
    df_copy['temp_p'] = df_copy['시간대'].apply(get_meal_priority)
    df_sorted = df_copy.sort_values(by=["날짜", "temp_p"]).reset_index(drop=True)
...
if not all_data.empty:
    ...
    # 요약 UI
    st.info(f"💳 총 사용 금액: {total_sum:,}원 | 💰 남은 한도: {remaining_amount:,}원")

    # 삭제 로직
    checked_indices = edited_data[edited_data["삭제체크"] == True].index.tolist()
    if checked_indices and st.button("🗑️ 선택 항목 삭제"):
        remaining_df = all_data.drop(checked_indices).reset_index(drop=True)
...
if not done_df.empty:
    ...
    with d2:
        pdf_fn = f"{datetime.now().month}월_법카영수증.pdf"
        st.download_button("📄 PDF 다운로드", create_photo_pdf(done_df), pdf_fn)
